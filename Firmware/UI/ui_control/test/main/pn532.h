...
/**
 * @brief Unlocks a Gen1a "magic" Mifare Classic card and rewrites block 0
 *        (UID + BCC + SAK + ATQA + manufacturer bytes).
 * @details Call this immediately after pn532_wait_for_card() +
 *          pn532_read_detected_passive_target() succeed on the target card.
 *          Only works on Gen1a magic cards (backdoor 0x40/0x43 unlock).
 *          Genuine Mifare Plus / Mifare Classic cards will simply fail this
 *          (block 0 is factory-locked) -- this is expected and not a bug.
 *
 * @param uid      4-byte UID to write.
 * @param sak      SAK byte to write.
 * @param atqa     2-byte ATQA, MSB first as you'd normally write it (e.g. {0x00,0x04}).
 * @param mfg_data 8 bytes of manufacturer data for bytes 8..15 of block 0
 *                 (pass NULL to zero-fill).
 * @return ESP_OK on success, ESP_FAIL if unlock or write failed.
 */
esp_err_t pn532_magic_write_block0(const uint8_t uid[4], uint8_t sak,
                                    const uint8_t atqa[2], const uint8_t *mfg_data);
