...
/* Actual hardware wiring: IRQ line runs to GPIO32.
 * NOTE: GPIO32 was previously double-booked as PIN_CS_ADC in spi.c --
 * that's been moved to GPIO13 (see spi.c) to free this pin for the IRQ. */
#define RFID_IRQ_PIN GPIO_NUM_32
...
esp_err_t pn532_set_power_mode(void) {
    /* NOTE: this used to build a WRITEREGISTER frame that (a) manually stuffed
     * a 0xD4 host-to-PN532 byte into pn532_packetbuffer[1] even though
     * _write_full_command_frame() already inserts that byte itself, and
     * (b) told _send_command_wait_ack_irq() the frame was only 1 byte long,
     * so the real register address + value were never actually transmitted.
     * The chip would see a bare, malformed WRITEREGISTER command, which can
     * leave it in an undefined state and desync every command that follows
     * (SAM config, firmware version, InListPassiveTarget). Since this step
     * isn't required to read UIDs and we don't have a verified-correct
     * register/value for this hardware revision, it's disabled rather than
     * risk sending another malformed frame. Re-enable only with a frame
     * built the same way pn532_soft_reset()/pn532_ntag_write_page() build
     * theirs: packetbuffer[0]=CMD, then real payload bytes, and cmdlen equal
     * to the actual number of bytes filled in. */
    return ESP_OK;
}
